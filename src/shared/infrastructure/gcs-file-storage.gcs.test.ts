...
function uploadHeaders(size: number) {
  return {
...
test("署名したサイズより大きいファイルは拒否される", async () => {
  ...
  expect(await upload(url, larger, uploadHeaders(pdf.length))).not.toBe(200);
});
...
test("署名と違う種類のファイルは拒否される", async () => {
  ...
  const status = await upload(url, pdf, {
    ...uploadHeaders(pdf.length),
...
test("サイズの範囲のヘッダーを付けないと拒否される", async () => {
  ...
  const status = await upload(url, pdf, { "content-type": "application/pdf" });
  expect(status).not.toBe(200);
});
