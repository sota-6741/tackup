...
- プロジェクトの決まりは `AGENTS.md` と `.claude/rules/` にある。特に次の点に沿っているかを見る。
  ...
  - エラーハンドリングは `.claude/rules/error-handling.md` に沿う。正常な不存在は `null`、想定内の失敗は `reason` を持つ型付きの結果で返し、Server Action で `{ error }` の状態に変える。例外は Infrastructure の障害と想定外の失敗だけに使い、意味のない catch をしない。新しいコードで想定内の失敗に `DomainError` を投げない。
