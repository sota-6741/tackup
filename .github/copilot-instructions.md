...
- プロジェクトの決まりは `AGENTS.md` にある。特に次の点に沿っているかを見る。
  - 依存は内側（domain）に向かうだけ。presentation は infrastructure を import しない。use case は `@/di/*` から受け取る。
  ...
  - テスト名は日本語。
- `src/shared/presentation/components/ui/**`（shadcn/ui が生成）と `drizzle/meta/**`（drizzle-kit が生成）は、手で直さないのでレビューしない。
